\section{Data Summary \& Analysis Strategy}  \label{sec:features}
...
system version in the data set to aid in correlation.}

\subsection{Identifying regions of interest} \label{sec:features/partitioning}
...
$w$ was chosen to be a multiple of seven days to align with the U.S. work
week and ensure that weekends and weekdays were equally represented both SMA
calculations.  $w_{long}$ was set to 7 weeks to span multiple $w_{short}$
...
\TODO{Possibly redundant, but reading to this point without having read the
next section, it would be helpful to have the top half of Figure 5 here to
illustrate a shaded region bounded by crossover points.  Maybe split it, or
repeat it?}

In general we find that \textbf{existing work in financial market technical
analysis can be adapte to
time-varying I/O performance data to attenuate noise and identify regions of
interest based on performance trends.}
