...
In the following analyses, we separate our 11,986 performance observations into sets of observations that all ran on the same test platform (as described in Table \ref{tab:platform-descriptions}) to characterize the factors that contribute to time-dependent\textcolor{blue}{, systemic} performance variation across different file systems and architectures.
...
Section~\ref{sec:features/partitioning}.  \textcolor{blue}{Using performance observations from all \mira
...
To apply this method, we first group observations into sets of data by the test platform on which they ran as was described previously.
...
490 anomalous observations, 410 of which have at least one positively classified attribute.

As described in Section \ref{sec:methods/tokio}, not every observation's feature vector has every feature defined.
So as not to bias our results away from those attributes that were only measured for a subset of observations, we express the importance of each attribute as the number anomalous observations where it was positively classified to the number where it was measured.
...
On the contrary, contention for IOPS is also positively classified in a significant fraction of anomalous observations despite it not arising as a factor in longer-term performance divergence.
...
Conversely, we can conclude that periods of high IOPS contention do not last long enough to be implicated in long-term performance degradation on production file systems, whereas the same is not true of bandwidth contention.
...
This result also underscores the fact that this transient classification provides different insights into performance variation from the longer-term correlative analysis.
...
These findings indicate that, like IOPS contention, metadata contention is much more likely to coincide with transient performance loss than a long-term and sustained divergence of performance.
