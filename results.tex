...
regions with fewer than three data points. The small regions are discarded
...
We can now apply Pearson correlation to the feature vectors within a divergence
region in order to identify factors that contributed to the performance
...
This also distinguishes this utility of this transient classification from the longer-term correlative analysis and underscores the fact that the sustained bandwidth contention identified in Section \ref{sec:results/correlate-all} is indeed a distinct phenomenon from the transient contention identified here.
...
To ascertain whether the rate at which these three attributes were classified is statistically significant, we can assign p-values to the frequency of occurrence stated for each attribute in Figure \ref{fig:contributors-bad-by-system}.

% leverage the fact that we have a number of positive and negative classifications and p-values for each divergence region to 

we calculate the probability of reaching that result 

, we can apply the binomial test to each fraction expressed in Fig. \ref{fig:contributors-bad-by-system} to calculate p-values for those distributions.

% Although the statistical significance of each individual classification is generally poor due to our generous choice of $\textup{p-value} < 0.10$, 
