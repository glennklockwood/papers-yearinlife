...
Two factors of this method are worth noting:

\begin{itemize}
\item Although the statistical significance of each individual classification is generally poor due to our generous choice of $\textup{p-value} < 0.10$, expressing the positively classified attributes as aggregate fractions allows us to assert much greater statistical significance.
Since we have a number of positive and negative classifications as well as a p-value for each divergence region, we can apply the binomial test to each fraction expressed in Fig. \ref{fig:contributors-bad-by-system} to calculate a p-value
