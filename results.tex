...
We then apply Pearson correlation to the feature vectors within
...
for each divergence region which contains the correlation coefficients between the fraction of peak performance and every other
attribute across all observations in that region.  We then use the p-value of each correlation coefficient to further downselect the total set of regions to those 
...
% \\
% gkl- hopefully addressed in Fig \ref{fig:mira-regions-overview}}
...
% \\
% gkl- I asked myself the same Q.  It turns out that a correlation between exactly two data points produces a perfect p-value of 0.0 because you can draw a straight line through those two points.  This passes the significance filter but royally messes up the results.}

\TODO{I'm not sure I understand why we're aggregating trend regions from different motifs here, since everywhere else our analysis is applied to these independently. For one, different motifs could have different trend regions by definition. If the results hold over just one motif, maybe just use those, or something? -SS}
...
particular divergence region in November 2017 on \mira \mirafsone.  This example
...
results shows why: this region contains a cluster of poorly performing
probes (${0.2 < \textup{fraction peak perf} < 0.4}$) that ran with a
relatively high bandwidth coverage factor.  This region is also the
single largest divergence region observed on \mira; a difference of over 20\%
 between $\textup{SMA}_{short}$ and $\textup{SMA}_{long}$ was
...
\TODO{Are we sure we only want to determine one and only one anomalous observation in each trend region? -SS}
...
Quantitatively, this simple classification scheme allows us to identify relationships and quantify the statistical significance of each classification for individual anomalous observations.
To establish the statistical significance, we calculate the p-value of each classification in a divergence region to be $\textup{p-value} = \frac{1}{N}$, where $N$ is the number of observations in the region; intuitively, this value is the probability of a random variable being positively classified.
