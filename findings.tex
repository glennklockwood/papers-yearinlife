...
We have found that the combination of system-wide telemetry, active performance
...
codified successful techniques, such adaptations of financial analysis
strategies, into our open source framework for reuse on other systems
(see Appendix \ref{sec:appendix/artifacts}).  We believe that the state of the
practice in HPC storage will be advanced by making this methodology
readily available and repeatable on other platforms.  Our framework notably
decouples analysis techniques from data integration so that the
the same analysis tools can be reused on any platform once modular
connectors have been added to normalize that platform's telemetry data.

We have also further advanced the state of the practice in this study by uncovering novel
insights into the nature of I/O performance in production storage systems.
In this section, we revisit the most significant outcomes and
