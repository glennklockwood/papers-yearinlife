...
For data sources that produce time-resolved data (e.g., LMT reports workload statistics at five-second intervals), we calculate the sum, minimum, maximum, and average value of the data generated over the duration of the job and define those reduced scalars as attributes of the feature vector.
...
For example, a job's \emph{bandwidth coverage factor} is the number of bytes read and written by that job (measured by Darshan) divided by the number of bytes read and written across the entire storage system (measured by LMT or ggiostat) while that job was running.
