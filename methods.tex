...
described in Table~\ref{tab:platform-descriptions}\footnote{The same
...
processes) on \edison, and 1,024 nodes (16,384 processes) on \mira.  On
Lustre, the
shared-file benchmarks used wide striping while the file-per-process
benchmarks disabled striping.  In either case the goal was to leverage as
many storage devices as possible in aggregate for each benchmark.
...
usage.  These samples (which are not used in this study) point out the need
