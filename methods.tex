...
In the following sections, we provide a brief  overview of \tokio, describe the production HPC platforms we have 
%deployed it on, 
used in this study,
and describe our methodology for using a suite of daily I/O performance probes for active sampling of file system performance.
...
A coverage factor of 1.0 indicates that a job was the exclusive consumer of a resource, while a coverage factor of 0.4 indicates that other competing jobs consumed 60\% of the total delivered resources.

In this study, each job has a feature vector of 220 attributes, although some attributes are derived from others and therefore degenerate.
In addition, some attributes are not defined on some systems due to those systems' monitoring modules not reporting certain metrics.
