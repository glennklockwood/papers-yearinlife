...
be sustained over production system life spans measured in years.

In this work we seek to advance the state of the practice in understanding
and diagnosing large-scale I/O performance by developing techniques
for identifying regions of interest in I/O performance data as well
as differentiated techniques for analyzing long-term trends and
transient performance anomalies.  These techniques are demonstrated
on a comprehensive I/O performance data set assembled via multiple
continuous instrumentation methods across a full year of production
storage activity on two leadership-scale computing facilities.  This data
set affords unusual opportunity to investigate the relationship between
point-in-time observations and long-term behavior.

By applying our analysis techniques to a year in the life of a parallel file
system, we observe both transient and long term performance trends and
discover that each is often correlated with different contributing
factors.  Overall we find that large-scale I/O performance and variability
is not constant, and instrumentation, analysis methods, and performance
models must be adapted to prevailing conditions in order to effectively derive
actionable insight from I/O instrumentation.
